...
-- ==============================================================
CREATE PROCEDURE [dbo].[CronWordTests]
AS
...
SELECT * FROM (
	SELECT Expected, dbo.CronWord(Expression, Separator, [Index]) Actual,
		Expression + '_' + ISNULL(Separator, ' ') + '_' + CAST([Index] AS varchar) Func
	FROM @Tests
) T
